...
| `W2_adopt_job` | SECOND adoption route, by job identity instead of placement: the spawned claude carries `HERD_JOB` in its **environment** (`launch.py --env`), so it can say which reservation is its own. Exists because the window stamp may not have landed yet — `W1_spawn_window` takes the write lock while the launched claude is already starting, and if SessionStart wins that race `W2_adopt` matches nothing, `W2b_insert` makes a duplicate, and W3f sweeps the reservation with the job name 120s later. Ordered `session_pk DESC LIMIT 1` (newest wins) and filters `session_id IS NULL` in its own subquery, so it can never target an already-adopted row. See [spawn identity](DECISIONS.md#spawn-identity). |
