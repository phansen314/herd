...
-- Enforced by test_source_invariants.py. tier2->tier1 writes ok, tier1->tier2 forbidden.
--
-- WRITERS: session_start.sh, session_end.sh, stop.sh, notification.sh,
-- post_tool_use.sh (HOT PATH), statusline.sh (metrics + updated_at ONLY, never
-- last_event_*), daemon (stopped_at on silent death, W3d/W3e).
...
CREATE TABLE IF NOT EXISTS sessions (
    -- AUTOINCREMENT (not bare rowid) so the surrogate is never reused once a real
    -- DELETE/prune lands. See DESIGN.md#identity.
    ...
    git_worktree        TEXT,          -- linked-worktree NAME, NULL in a main working tree.
                                       -- Unlike worktree.*, set for ANY `git worktree add`
                                       -- checkout, not just --worktree runs.

    -- session_name is Claude's /rename name, user-mutable — deliberately NOT
    -- herd's job_name (aliasing would break the handle).
    session_name        TEXT,
    model               TEXT,
    ...
    -- status_source is observation provenance only:
    -- hook (reported) | reconcile (discovery inferred) | pid (liveness inferred).
    status              TEXT NOT NULL DEFAULT 'unknown'
                        CHECK (status IN ('working','waiting','needs_approval',
                                          'stopped','unknown')),
    status_source       TEXT CHECK (status_source IN ('hook','reconcile','pid')),

    -- DO NOT CONFLATE the two clocks: last_event_at is semantic activity from
    -- lifecycle hooks ONLY, updated_at is any write. The gap is the attention
    -- signal, so statusline MUST NOT touch last_event_*. DESIGN.md#two-clocks.
    last_event_at       TEXT,
    ...
    -- NOT cumulative totals despite the names: since Claude Code v2.1.132 these are
    -- what is currently IN the context window — a gauge, not a counter. Named to
    -- match klawde's schema.
...
-- At most one LIVE session per pid — makes the identity merge safe by construction.
-- Dead rows fall out, so pid reuse after a clean end is fine; reboot pid-reuse is
-- closed by the boot sweep (W3e). See DESIGN.md#pid.
